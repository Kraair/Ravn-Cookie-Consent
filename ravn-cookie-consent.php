...
require_once RAVN_PLUGIN_DIR . 'includes/class-ravn-admin.php';
require_once RAVN_PLUGIN_DIR . 'includes/class-ravn-frontend.php';
...
function ravn_activate_plugin() {
	RAVN_DB::create_tables();
	RAVN_DB::maybe_seed_default_categories();
...
function ravn_run_daily_cleanup() {
	...
	if ( $months > 0 ) {
		RAVN_DB::purge_old_consent_logs( $months );
	}
}
...
function ravn_maybe_upgrade() {
	...
	if ( $installed_version !== RAVN_VERSION ) {
		RAVN_DB::create_tables();
		update_option( 'ravn_db_version', RAVN_VERSION );
	}
	if ( ! wp_next_scheduled( 'ravn_daily_cleanup' ) ) {
		wp_schedule_event( time(), 'daily', 'ravn_daily_cleanup' );
	}
